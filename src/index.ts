...
// Built-in adapter auto-registration lives in ./builtin-adapters.ts and is
// reached ONLY through the dynamic import in `initializeProvider` below — never
// statically from this module. That keeps the 46 provider `import()` targets
// out of the chunk a bundler traces from `import { Meridian }`, so consumers
// wiring explicit adapters can drop them all (mark `builtin-adapters` external).
// The provider-name list and loaders are re-exported from that module, not here.
...
export class Meridian {
  ...
  private async initializeProvider(
    ...
    if (!adapter) {
      // Dynamic import: the built-in loader table is not statically reachable
      // from the Meridian class, so a bundler need not pull all 46 adapters
      // into the entry chunk. Resolves once, then module-cached by the runtime.
      const { getBuiltinAdapter } = await import("./builtin-adapters.js");
